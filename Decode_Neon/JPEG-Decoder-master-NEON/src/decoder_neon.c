...
/*  
    函数功能：以16位二进制位为单元向BMP文件文件中写入图像数据
    函数输入：文件名；数据量
    函数输出：无
*/
...
void anti_q (BLOCK obj, DQT* quant) {
    ...
    float32x4_t obj_my = vdupq_n_f32(0.0);
    uint16x8_t quantizer_my = vdupq_n_u16(0);
    uint16x8_t mask_FF = vdupq_n_u16 (0x00FF);
    float tmp1[64];
    float32x4_t tmp = vdupq_n_f32(0.0);
    // 8位量化系数
    if (quant->precise == 0) {
        for(int i = 0; i < 8; ++i) {
            quantizer_my = vld1q_u16(quant->quantizer + 8 * i);
            quantizer_my = vandq_u16(quantizer_my, mask_FF);
            vst1q_u16(quant->quantizer + 8 * i, quantizer_my);
        }
        
        for(int i = 0; i < 64; ++i) {
            tmp1[i] = (float)quant->quantizer[i];
        }
        tmp = vld1q_f32(tmp1);
        for(int i = 0; i < 16; ++i) {
            obj_my = vld1q_f32(obj + 4 * i);
            obj_my = vmulq_f32(obj_my, tmp);
            vst1q_f32(obj + 4 * i, obj_my);
        }
    }
    // 16位量化系数
...
void fst_IDCT (BLOCK des, BLOCK src) {
    ...
    // 初始余弦系数
    if (!cos_table) {
        cos_table = malloc(sizeof(float) * M_COS);
        float num[M_COS], res[M_COS];
        for(int i = 0; i < M_COS; ++i) {
            num[i] = (float)i;
        }
        float32x4_t res_my = vdupq_n_f32(0.0);
        for(int n = 0; n < 50; ++n) {
            res_my = vld1q_f32(num + 4*n);
            res_my = vmulq_n_f32(res_my, M_PI);
            vst1q_f32(res+4*n, res_my);
        }
        for(u8 i = 0; i < M_COS; ++i) {
            cos_table[i] = cos(res[i] / 16.0);
        }
    }
   // 初始常量系数
    if (!coeff) {
        coeff = malloc(sizeof(float) * 8);
        coeff[0] = 1. / sqrt(2.);
        for (u8 i = 1; i < 8; i++)
            coeff[i] = 1.;
    }
    ...
    // 余弦变换
    memset(des, 0, sizeof(float) * N_QUANT);
    for (int j = 0; j < 8; j++)
        for (int x = 0; x < 8; x++) {
            for (int y = 0; y < 8; y++)
...
void upsampling(MCU* obj, JPEG_INFO* info) {
    ...
    u8 idx_up;
        u8 idx_x_h;
        uint8x8_t y_my = vdup_n_u8(0);
        uint8x8_t y_my1 = vdup_n_u8(0);
        uint8x8_t l_my = vdup_n_u8(0);
        l_my = vld1_u8(num_0_7_u8);
        uint8x8_t idx_BLOCK_my = vdup_n_u8(0);
        uint8x8_t idx_BLOCK_my1 = vdup_n_u8(0);
        uint8x8_t idx_x_y = vdup_n_u8(0);
        uint8x8_t idx_x_y1 = vdup_n_u8(0);
        u8 idx_BLOCK[8];
        u8 idx_x_y_my[8];
    ...
    for(u8 chan = 1; chan <= 3; ++chan) {
        ...
        u8 v_idx = info->CHAN[chan]->v_factor / info->MCU_v_factor;
        u8 h_idx = info->CHAN[chan]->h_factor / info->MCU_h_factor;
        u8 idx_x_h1 = chan * N_CHAN;
        tmp = vdup_n_u8(h_idx);
        y_my1 = vmul_u8(l_my, tmp);
        for (u8 i = 0; i < info->MCU_v_factor; ++i) {
            for (u8 j = 0; j < info->MCU_h_factor; ++j) {
                idx_up = info->MCU_h_factor * i + j;
                tmp = vdup_n_u8(8 * j * h_idx);
                y_my = vadd_u8(y_my1, tmp);
                idx_BLOCK_my = vshr_n_u8(y_my, 3);
                tmp = vdup_n_u8(8);
                idx_x_y = vmul_u8(idx_BLOCK_my, tmp);
                idx_x_y = vsub_u8(y_my, idx_x_y);
                for (u8 k = 0; k < 8; ++k) {
                    u8 x = (8 * i + k) * v_idx;
                    idx_x_h = (x / 8) * info->CHAN[chan]->h_factor + idx_x_h1;
                    tmp = vdup_n_u8(idx_x_h);
                    idx_BLOCK_my1 = vadd_u8(idx_BLOCK_my, tmp);
                    vst1_u8(idx_BLOCK, idx_BLOCK_my1);
                    tmp = vdup_n_u8(8 * (x % 8));
                    idx_x_y1 = vadd_u8(idx_x_y, tmp);
                    vst1_u8(idx_x_y_my, idx_x_y1);
                    for (u8 l = 0; l < 8; ++l) {
                        (*up[idx_up])[8 * k + l] = (*obj->BLOCK[idx_BLOCK[l]])[idx_x_y_my[l]];
                    }
                }
            }
        }
        for (u8 i = 0; i < MCU_N_BLOCK; ++i)
            obj->BLOCK[i + idx_x_h] = up[i];
    }
...
RGB* anti_trans_color (JPEG_INFO* info, JPEG_DATA* data) {
    ...
    float32x4_t Y_neon = vdupq_n_f32(0.0);
    float32x4_t C1_neon = vdupq_n_f32(0.0);
    float32x4_t C2_neon = vdupq_n_f32(0.0);
    float32x4_t res0_neon = vdupq_n_f32(0.0);
    float32x4_t res1_neon = vdupq_n_f32(0.0);
    float32x4_t res2_neon = vdupq_n_f32(0.0);
    float32x4_t res3_neon = vdupq_n_f32(0.0);

    for (u8 i = 0; i < info->MCU_NV; ++i) {
        for (u8 j = 0; j < info->MCU_NH; ++j) {
            u16 idx_MCU   = info->MCU_NH * i + j;
            for (u8 k = 0; k < info->MCU_v_factor; ++k) {
                x0 = 8 * info->MCU_v_factor * i + 8 * k;
                for (u8 l = 0; l < info->MCU_h_factor; ++l) {
                    y0 = 8 * info->MCU_h_factor * j + 8 * l;
                    ...
                    for (u8 m = 0; m < 8; ++m) {
                        ...
                        for (u8 n = 0; n < 8; ++n) {
                            ...
                            float Y  = (*data->MCU[idx_MCU]->BLOCK[idx(1, idx_BLOCK, N_CHAN)])[idx_px];
                            float C1 = (*data->MCU[idx_MCU]->BLOCK[idx(2, idx_BLOCK, N_CHAN)])[idx_px];
                            float C2 = (*data->MCU[idx_MCU]->BLOCK[idx(3, idx_BLOCK, N_CHAN)])[idx_px];
                            index_my[cnt] = index;
                            Y_my[cnt] = Y;
                            C1_my[cnt] = C1;
                            C2_my[cnt] = C2;
                            ...
                            if(cnt == 4) {
                                cnt = 0;
                                Y_neon = vld1q_f32(Y_my);
                                C1_neon = vld1q_f32(C1_my);
                                C2_neon = vld1q_f32(C2_my);
                                res0_neon = vmulq_n_f32(Y_neon, 1.0);
                                float32x4_t tmp = vdupq_n_f32(128.0);
                                res0_neon = vaddq_f32(res0_neon, tmp);
                                res1_neon = vmulq_n_f32(C2_neon, 1.402);
                                res1_neon = vaddq_f32(res1_neon, res0_neon);
                                res3_neon = vmulq_n_f32(C1_neon, 1.772);
                                res3_neon = vaddq_f32(res3_neon, res0_neon);
                                res2_neon = vmulq_n_f32(C1_neon, -0.34414);
                                res2_neon = vaddq_f32(res2_neon, res0_neon);
                                res0_neon = vmulq_n_f32(C2_neon, -0.71414);
                                res2_neon = vaddq_f32(res2_neon, res0_neon);
                                vst1q_f32(res, res1_neon);
                                vst1q_f32(res + 4, res2_neon);
                                vst1q_f32(res + 8, res3_neon);
                                while(cnt != 4) {
                                    img->R[index_my[cnt]] = bandpass(res[cnt]);
                                    img->G[index_my[cnt]] = bandpass(res[cnt + 4]);
                                    img->B[index_my[cnt]] = bandpass(res[cnt + 8]);
                                    cnt++;
                                }
                                cnt = 0;
                            }
...
BLOCK* read_BLOCK (JPEG_INFO* info, u8 chan_id) {
    ...
    while (idx < 64) {
        ...
        if (symbol_len == 0x00) {// 读取到文件结束标志位
            ...
            while(idx < 64) {
                vst1q_f32((*obj) + idx, obj_my);
                idx += 4;
            }
        }
        else if (symbol_len == 0xF0) {
            for (int i = 0; i < 4; ++i) {
                vst1q_f32((*obj) + idx + 4 * i, obj_my);
            }
            idx += 16;
        }
        else {
